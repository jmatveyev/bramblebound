...
try:
 with sync_playwright() as p:
  ...
  page=b.new_page(viewport={'width':844,'height':390},has_touch=True,is_mobile=True); errors=[];logs=[]
  ...
  try:
   result=page.evaluate((Path(__file__).resolve().parent / 'route-touch.js').read_text());result['errors']=errors;result['console']=logs;(Path(__file__).resolve().parent / 'route-touch-report.json').write_text(json.dumps(result,indent=2));print(json.dumps({'passed':result['passed'],'failed':result['failed'],'errors':errors,'console':logs},indent=2))
  except Exception as e:
   print('ROUTE ERROR',e);print(page.evaluate('({state:__BB.state,room:__BB.room,player:__BB.player,data:__BB.data,world:__BB.world.boss})'))
  b.close()
finally:x.terminate()

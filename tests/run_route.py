...
try:
 with sync_playwright() as p:
  ...
  try:
   result=page.evaluate((Path(__file__).resolve().parent / 'route.js').read_text());result['errors']=errors;result['console']=logs;(Path(__file__).resolve().parent / 'route-report.json').write_text(json.dumps(result,indent=2));print(json.dumps({'passed':result['passed'],'failed':result['failed'],'errors':errors,'console':logs},indent=2))
